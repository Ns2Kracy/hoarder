<script lang="ts">
  ...
  function pageFromHash(): PageId {
    ...
    return navigation.some((item) => item.id === hashPage) ? (hashPage as PageId) : "overview";
  }
</script>
...
<div class="min-h-screen bg-zinc-100 text-zinc-900">
  <div class="flex min-h-screen">
    <aside class="hidden w-60 shrink-0 border-r border-zinc-200 bg-white lg:block">
      <div class="flex h-12 items-center gap-2 border-b border-zinc-200 px-3">
        <FolderCog aria-hidden="true" size={19} class="text-blue-700" />
        <div>
          <p class="text-sm font-semibold leading-4 text-zinc-950">Hoarder</p>
          <p class="text-xs leading-4 text-zinc-500">Connector Console</p>
        </div>
      </div>
    ...
    {#if sidebarOpen}
      ...
      ></button>
      <aside class="fixed inset-y-0 left-0 z-30 w-64 border-r border-zinc-200 bg-white lg:hidden">
        <div class="flex h-12 items-center gap-2 border-b border-zinc-200 px-3">
          <FolderCog aria-hidden="true" size={19} class="text-blue-700" />
          <div>
            <p class="text-sm font-semibold leading-4 text-zinc-950">Hoarder</p>
    ...
    <div class="min-w-0 flex-1">
      <header class="sticky top-0 z-10 border-b border-zinc-200 bg-white">
        <div class="flex h-12 items-center justify-between gap-3 px-3 lg:px-4">
          <div class="flex min-w-0 items-center gap-2">
            ...
            <div class="min-w-0">
              <p class="truncate text-sm font-semibold text-zinc-950">Local API · 127.0.0.1:4761</p>
              <p class="truncate text-xs text-zinc-500">
                {$consoleOrigin === "api" ? "Live API data" : "Mock data fallback"} · refreshed
                {formatDateTime($sources.updatedAt ?? $jobs.updatedAt ?? $runs.updatedAt)}
              </p>
            </div>
          </div>
      ...
      <main class="mx-auto max-w-7xl p-3 lg:p-4">
        {#if activePage === "overview"}
          <Overview summary={$summary} sources={$sources} jobs={$jobs} runs={$runs} onRefresh={loadConsoleData} />
        {:else if activePage === "sources"}
          <Sources sources={$sources} onAddSource={addSource} onTestSource={testSourceConnection} />
