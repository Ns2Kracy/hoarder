...
<section class="space-y-4">
  ...
  <section class="rounded border border-zinc-200 bg-white">
    <div class="flex items-center justify-between gap-3 border-b border-zinc-200 px-3 py-2">
      ...
      <span class="text-xs text-zinc-500">{jobs.data.filter((job) => job.enabled).length} enabled</span>
    </div>

    {#if jobs.data.length === 0}
      <div class="px-3 py-8 text-sm text-zinc-500" role="status">No sync jobs configured.</div>
    {:else}
          ...
          <tbody class="divide-y divide-zinc-100">
            {#each jobs.data as job (job.id)}
              <tr class="hover:bg-zinc-50">
                <td class="px-3 py-2">
                  <p class="font-medium text-zinc-900">{job.sourceName}</p>
                  <p class="font-mono text-xs text-zinc-500">{job.id}</p>
                </td>
                <td class="px-3 py-2 text-zinc-700">{job.schedule}</td>
                <td class="px-3 py-2">
                  <StatusBadge status={job.status} />
                </td>
                <td class="whitespace-nowrap px-3 py-2 text-zinc-600">{formatDateTime(job.lastRunAt)}</td>
